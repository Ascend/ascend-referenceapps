                                ...
                                VpcProcessType processType)
{
    ...
    // The cropped image will be pasted on the upper left corner or the middle location or the whole location according
    // to the processType
...
APP_ERROR DvppCommon::CombineVdecProcess(std::shared_ptr<DvppDataInfo> data, void *userData)
{
    ...
    ret = GetVideoDecodeStrideSize(dataInfo.width, dataInfo.height, dataInfo.format, dataInfo.widthStride,
        dataInfo.heightStride);
    if (ret != APP_ERR_OK) {
