...
IndexSQIP::IndexSQIP(int dim, int resourceSize) : IndexSQ(dim, resourceSize)
{
    resetSqDistOperator();
}
...
void IndexSQIP::searchImpl(int n, const float16_t *x, int k, float16_t *distances, idx_t *labels)
{
    ...
    AscendTensor<float16_t, DIMS_3> maxDistResult(mem, {batchNum, n, this->burstsOfComputeBatch}, stream);
    opFlag.zero();
    ...
    auto topkFunctor = [&](int idx) {
        ...
        for (int i = 0; i < batchNum && !errorQuit; ++i) {
            ...
            for (int j = idx; j < n; j += THREADS_CNT) {
                ...
                if (i == 0) {
                    ASCEND_THROW_IF_NOT(topkOp.exec(opOutTp, topkHeapTp, maxHeapTp, size, BURST_LEN));
                } else {
    ...
    for (int i = 0; i < batchNum; ++i) {
        ...
        AscendTensor<float16_t, DIMS_2> maxResult = maxDistResult[i].view();

        int offset = i * this->distComputeBatch;
        actualSize[0] = std::min(static_cast<uint32_t>(this->ntotal - offset),
                                 static_cast<uint32_t>(this->distComputeBatch));

        runSqDistOperator(queries, batchCode, this->vDiff, this->vMin, actualSize, result, maxResult, flag, stream);
    }
...
void IndexSQIP::runSqDistOperator(AscendTensor<float16_t, DIMS_2> &queries,
...
void IndexSQIP::resetSqDistOperator()
{
    auto sqDistOpReset = [&](std::unique_ptr<AscendOperator> &distSqOp, int batch) {
        distSqOp.reset();
        AscendOpDesc desc("DistanceSQ8IPMaxs");
