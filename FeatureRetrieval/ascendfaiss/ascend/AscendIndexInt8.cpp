...
AscendIndexInt8::AscendIndexInt8(int dims, faiss::MetricType metric, AscendIndexInt8Config config)
    : d(dims), metric_type(metric), indexConfig(config), pool(nullptr)
{
...
void AscendIndexInt8::add(Index::idx_t n, const char *x)
{
    const int8_t *xi = reinterpret_cast<const int8_t *>(x);
    return add_with_ids(n, xi, nullptr);
}

void AscendIndexInt8::add_with_ids(Index::idx_t n, const char *x, const Index::idx_t *ids)
{
    const int8_t *xi = reinterpret_cast<const int8_t *>(x);
    return add_with_ids(n, xi, ids);
}

void AscendIndexInt8::add(Index::idx_t n, const int8_t *x)
{
    return add_with_ids(n, x, nullptr);
}

void AscendIndexInt8::add_with_ids(Index::idx_t n, const int8_t *x, const Index::idx_t *ids)
{
    FAISS_THROW_IF_NOT_MSG(x, "x can not be nullptr.");
    FAISS_THROW_IF_NOT_MSG(this->is_trained, "Index not trained");
    FAISS_THROW_IF_NOT_FMT(n <= (Index::idx_t)std::numeric_limits<int>::max(), "indices exceeds max(%d)",
...
void AscendIndexInt8::train(Index::idx_t n, const int8_t *x)
{
    VALUE_UNUSED(n);
    VALUE_UNUSED(x);
}

void AscendIndexInt8::updateCentroids(Index::idx_t n, const int8_t *x)
{
    VALUE_UNUSED(n);
    VALUE_UNUSED(x);
}

void AscendIndexInt8::updateCentroids(Index::idx_t n, const char *x)
{
    const int8_t *xi = reinterpret_cast<const int8_t *>(x);
    updateCentroids(n, xi);
}

void AscendIndexInt8::addPaged(int n, const int8_t *x, const Index::idx_t *ids)
{
    if (n <= 0) {
        return;
    }
    ...
    if (totalSize > ADD_PAGE_SIZE || static_cast<size_t>(n) > ADD_VEC_SIZE) {
        ...
        for (size_t i = 0; i < (size_t)n; i += tileSize) {
            ...
            if (this->verbose) {
                printf("AscendIndexInt8::add: adding %ld:%ld / %d\n", i, i + curNum, n);
            }
            addImpl(curNum, x + i * (size_t)this->d, ids ? (ids + i) : nullptr);
        }
    } else {
        if (this->verbose) {
...
void AscendIndexInt8::assign(Index::idx_t n, const int8_t *x, Index::idx_t *labels, Index::idx_t k)
{
...
void AscendIndexInt8::search(Index::idx_t n, const int8_t *x, Index::idx_t k, float *distances,
...
void AscendIndexInt8::search(Index::idx_t n, const char *x, Index::idx_t k, float *distances,
    Index::idx_t *labels) const
{
    std::vector<int8_t> xi(n * d);
    transform(x, x + n * d, xi.begin(), [](char elem) { return static_cast<int8_t>(elem); });
    return search(n, xi.data(), k, distances, labels);
}

void AscendIndexInt8::searchPaged(int n, const int8_t *x, int k, float *distances, Index::idx_t *labels) const
{
    if (n > 0) {
        ...
        if (totalSize > SEARCH_PAGE_SIZE || (size_t)n > SEARCH_VEC_SIZE || totalOutSize > SEARCH_PAGE_SIZE) {
            ...
            size_t maxNumVecsForPageSize = SEARCH_PAGE_SIZE / ((size_t)this->d * sizeof(float));

            size_t maxRetVecsForPageSize = SEARCH_PAGE_SIZE / (k * (sizeof(uint16_t) + sizeof(uint32_t)));
            ...
            for (int i = 0; i < n; i += tileSize) {
                ...
                searchImpl(curNum, x + i * (size_t)this->d, k, distances + i * k, labels + i * k);
            }
        } else {
...
void AscendIndexInt8::searchImpl(int n, const int8_t *x, int k, float *distances, Index::idx_t *labels) const
{
    size_t deviceCnt = indexConfig.deviceList.size();

    // convert query data from float to fp16, device use fp16 data to search
    std::vector<int8_t> query(n * this->d, 0);
    transform(x, x + n * this->d, begin(query), [](int8_t temp) { return temp; });
    ...
    auto searchFunctor = [&](int idx) {
        ...
        RpcError ret =
            RpcIndexInt8Search(ctx, indexId, n, this->d, k, query.data(), distHalf[idx].data(), label[idx].data());
...
void AscendIndexInt8::mergeSearchResult(size_t devices, std::vector<std::vector<float>> &dist,
...
void AscendIndexInt8::searchPostProcess(size_t devices, std::vector<std::vector<float>> &dist,
    std::vector<std::vector<uint32_t>> &label, int n, int k, float *distances, Index::idx_t *labels) const
{
