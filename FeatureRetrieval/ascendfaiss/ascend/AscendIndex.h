...
const int INDEX_DEFAULT_MEM = static_cast<int>(0x2000000); // 0x2000000 mean 32M(resource mem pool's size)

struct AscendIndexConfig {
    inline AscendIndexConfig() : deviceList({ 0 }), resourceSize(INDEX_DEFAULT_MEM) {}

    inline AscendIndexConfig(std::initializer_list<int> devices, int resources = INDEX_DEFAULT_MEM)
        : deviceList(devices), resourceSize(resources)
    {
        FAISS_THROW_IF_NOT_MSG(deviceList.size() != 0, "device list can not be empty!");
    }

    inline AscendIndexConfig(std::vector<int> devices, int resources = INDEX_DEFAULT_MEM)
        : deviceList(devices), resourceSize(resources)
    {
        FAISS_THROW_IF_NOT_MSG(deviceList.size() != 0, "device list can not be empty!");
    }
...
public:
    ...
    void compute_residual(const float *x, float *residual, Index::idx_t key) const override;

    // Computes a residual vector after indexing encoding(batch form).
    void compute_residual_n(Index::idx_t n, const float *xs, float *residuals, const Index::idx_t *keys) const override;
...
protected:
    ...
    virtual void addImpl(int n, const float *x, const Index::idx_t *ids) = 0;

    // Overridden to actually perform the search
    virtual void searchImpl(int n, const float *x, int k, float *distances, Index::idx_t *labels) const;
    ...
    void addPaged(int n, const float *x, const Index::idx_t *ids);
    ...
    void searchPaged(int n, const float *x, int k, float *distances, Index::idx_t *labels) const;
...
protected:
    ...
    friend class AscendIndexPreTransform;
};
