...
class DistanceIVFSQ8L2:
    def __init__(self,
                 ...
                 output_mins,
                 output_flag,
                 kernel_name="distance_ivf_sq8_l2"):
    ...
    def set_tik_instance(self):
        ...
        self.code_mins_loop = self.code_sub_loop // self.min_batch
    ...
    def cal_num_each_core(self):
        ...
        self.code_offset.set_as(code_retain % self.code_sub_loop // self.min_batch * self.min_batch)
        self.min_offset = self.tik_instance.Scalar(dtype="int32", name="min_offset")
        self.min_offset.set_as(code_retain % self.code_sub_loop // self.min_batch * 2)
    ...
    def distance_compute_each_loop(self, aicore_code_offset, aicore_code_num):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            self.tik_instance.data_move(queries_square_ub, queries_square_l0c,
                                        0, 1, self.queries_align * self.queries_align // 256, 0, 0)
            ...
            self.tik_instance.vcadd([0, 1], queries_l2_ub,
                                    queries_square_ub, 1, 1, 1, 8)
            ...
            self._add(queries_l2_ub,
                      queries_l2_ub, data_xdm_align_ub,
                      self.queries_align, self.fp16_mask)
            self.add_scalar.set_as(queries_l2_ub[0])
        ...
        code_square_l1 = self.tik_instance.Tensor("float32",
                                                  (self.code_num // 2,),
                                                  name="code_square", scope=tik.scope_cbuf)
        self.tik_instance.data_move(code_square_l1,
                                    self.input_precomputed_gm[aicore_code_offset],
                                    0, 1, (aicore_code_num + 7) // 8, 0, 0)
        ...
        with self.tik_instance.if_scope(code_loop_times_ > 0):
            with self.tik_instance.for_range(0, code_loop_times_) as loop_code:
                self.cube_compute_each_loop(inner_product_l0c, data_xd_l1, code_square_l1,
        ...
        if self.min_batch == 32:
            self.cube_compute_last_min32(inner_product_l0c, code_square_l1,
                                         aicore_code_offset, code_loop_times, code_last)
        else:
            self.cube_compute_last_min16(inner_product_l0c, code_square_l1,
                                         aicore_code_offset, code_loop_times, code_last)

    def cube_compute_each_loop(self, inner_product_l0c, data_xd_l1, code_square_l1,
        ...
        dst_min_ub = self.tik_instance.Tensor("float16",
                                              (self.code_mins_loop * 4,),
                                              name="dst_min_ub", scope=tik.scope_ubuf)
        with self.tik_instance.for_range(0, self.code_loops, thread_num=2) as loop_ub:
            # MTE2: move y^2 from l1 to UB
            code_l2 = self.tik_instance.Tensor("float32",
                                               (self.code_sub_loop,),
                                               name="code_l2", scope=tik.scope_ubuf)
            with self.tik_instance.if_scope(code_loop != 0):
                self.tik_instance.data_move(code_l2,
                                            code_square_l1[(code_loop - 1) * self.code_each_loop
            ...
            with self.tik_instance.if_scope(code_loop != 0):
                ...
                vcmin_repeat_times = self.code_sub_loop // self.min_batch
                self.tik_instance.vcmin(self.min_batch,
                                        dst_min_ub[(loop_ub % 2) * self.code_mins_loop * 2:],
            ...
            with self.tik_instance.if_scope(loop_ub % 2 * code_loop != 0):
                self.tik_instance.data_move(
                    ...
                    0, 1, self.code_sub_loop // 8, 0, 0)
                self.tik_instance.data_move(
                    self.output_mins_gm[0, (aicore_code_offset + code_loop * self.code_each_loop
                                            + (loop_ub - 3) * self.code_sub_loop) // self.min_batch * 2],
                    dst_min_ub,
                    0, 1, self.code_mins_loop // 4, 0, 0)
    ...
    def cube_compute_last_min32(self, inner_product_l0c, code_square_l1,
                                aicore_code_offset, code_loop_times, code_last):
        ...
        with self.tik_instance.for_range(1, 3, thread_num=2) as loop_ub:
            code_l2 = self.tik_instance.Tensor("float32",
                                               ...
                                               name="code_l2", scope=tik.scope_ubuf)
            self.tik_instance.data_move(code_l2,
                                        code_square_l1[code_loop_times * self.code_each_loop
                                                       - loop_ub * self.code_sub_loop],
                                        0, 1, self.code_sub_loop // 8, 0, 0)
            inner_product_ub = self.tik_instance.Tensor("float32",
                                                        (self.code_sub_loop // 16, self.queries_align, 16),
            ...
            with self.tik_instance.else_scope():
                vcmin_repeat_times = (code_last - self.code_sub_loop * code_last_loops) // self.min_batch
                with self.tik_instance.if_scope(vcmin_repeat_times > 0):
                    self.tik_instance.vcmin(self.min_batch,
                                            dst_min_ub, dst_ub,
                                            vcmin_repeat_times, 1, 1, self.min_batch // 16)
                code_last_retain = code_last % self.min_batch
                with self.tik_instance.if_scope(code_last_retain > 0):
                    self.tik_instance.vcmin(code_last_retain,
                                            dst_min_ub[self.min_offset],
            ...
            self.tik_instance.data_move(
                self.output_mins_gm[0,
                                    (aicore_code_offset + (code_loop_times - 1) * self.code_each_loop
                                     + (self.code_loops - loop_ub) * self.code_sub_loop) // self.min_batch * 2],
                dst_min_ub,
                0, 1, self.code_mins_loop // 8, 0, 0)

    def cube_compute_last_min16(self, inner_product_l0c, code_square_l1,
                                aicore_code_offset, code_loop_times, code_last):
        code_last_loops = code_last // self.code_sub_loop
        ...
        with self.tik_instance.for_range(0, self.code_loops, thread_num=2) as loop_ub:
            code_l2 = self.tik_instance.Tensor("float32",
                                               ...
                                               name="code_l2", scope=tik.scope_ubuf)
            self.tik_instance.data_move(code_l2,
                                        code_square_l1[(code_loop_times - 1) * self.code_each_loop
                                                       + loop_ub * self.code_sub_loop],
                                        0, 1, self.code_sub_loop // 8, 0, 0)
            inner_product_ub = self.tik_instance.Tensor("float32",
                                                        (self.code_sub_loop // 16, self.queries_align, 16),
            ...
            with self.tik_instance.if_scope(code_last_loops > loop_ub):
                vcmin_repeat_times = self.code_sub_loop // self.min_batch
                self.tik_instance.vcmin(self.min_batch,
                                        dst_min_ub[loop_ub * self.code_mins_loop * 2], dst_ub,
                                        vcmin_repeat_times, 1, 1, self.min_batch // 16)
            with self.tik_instance.else_scope():
                vcmin_repeat_times = (code_last - self.code_sub_loop * code_last_loops) // self.min_batch
                with self.tik_instance.if_scope(vcmin_repeat_times > 0):
                ...
                with self.tik_instance.if_scope(code_last_retain > 0):
                    self.tik_instance.vcmin(code_last_retain,
                                            dst_min_ub[loop_ub * self.code_mins_loop * 2 + self.min_offset],
                                            dst_ub[self.code_offset],
                                            1, 1, 1, 1)
            self.tik_instance.data_move(
                self.output_distances_gm[0,
...
def distance_ivf_sq8_l2(input_queries,
                        input_code,
                        input_precomputed,
                        input_diff,
                        input_min,
    ...
    None
    """
    distance_compute_sq8 = DistanceIVFSQ8L2(input_queries, input_code, input_precomputed,
                                            input_diff, input_min, input_actual_num,
                                            output_distances, output_mins, output_flag, kernel_name)
    tik_instance = distance_compute_sq8.get_tik_instance()
    return tik_instance
