...
ascend_index_ivfflat = ascendfaiss.AscendIndexIVFFlat(
    d, nlist, ascendfaiss.METRIC_L2, config)
ascend_index_ivfflat.verbose = True

# train
ascend_index_ivfflat.train(xb)

# add database
ascend_index_ivfflat.add(xb)

# search topk results
k = 10
display = 10
distance, labels = ascend_index_ivfflat.search(xq, k)
...
print("Remove top1")
num_removed = ascend_index_ivfflat.remove_ids(ids_remove)
...
num_removed = ascend_index_ivfflat.remove_ids(ids_remove_batch)
print("Removed num:", num_removed)
