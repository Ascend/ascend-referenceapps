...
public:
    ...
    inline float FExp(const float x) const
    {
        ...
        auto absQuantX = static_cast<uint32_t>(abs(quantX));
        float expx;
        if (quantX < 0) {
            expx = negCoef_[0][absQuantX & MASK_VALUE] * \
                   negCoef_[1][(absQuantX >> MASK_BITS) & MASK_VALUE];
        } else {
            expx = posCoef_[0][(absQuantX) & MASK_VALUE] * posCoef_[1][(absQuantX >> MASK_BITS) & MASK_VALUE];
        }
        return expx;
    }
    inline float FSigmoid(float x) const
    {
