...
APP_ERROR DetectPost::Process(std::shared_ptr<void> inputData)
{
    ...
    DetectPostProcess(singleResult, sendData->resizedData.width, sendData->resizedData.height, textInfo);
    ...
    if (sendData->itemNum == 0) {
        SendToNextModule(MT_WarpPerspective, sendData, instanceId_);
        LogInfo << "DetectPost[" << instanceId_ << "]: [" << sendData->imageName << "]: There is no text detected.";
    } else {
        SendAllTextBox(sendData, textInfo);
    }
...
void DetectPost::SendAllTextBox(std::shared_ptr<SendInfo> sendData, const std::vector<std::vector<int>> &textInfo)
{
                                       ...
                                       int &originX, int &originY)
{
    ...
    uint32_t paddingRight = (sendData->resizedData.width - (sendData->imageWidth / resizeRatio)) / halfValue;
    uint32_t paddingLeft = DVPP_ALIGN_UP(CONVERT_TO_EVEN(paddingRight), VPC_STRIDE_WIDTH);
    uint32_t resizeWidthStride = sendData->resizedData.width - paddingLeft - paddingRight;
    float resizeRatioWidth = static_cast<float>(sendData->imageWidth) / resizeWidthStride;
    originX = static_cast<uint32_t>((detectX - paddingLeft) * resizeRatioWidth);
