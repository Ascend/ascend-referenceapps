...
namespace {
    ...
    bool ShouldMerge(std::vector<std::pair<int, int>> region, std::pair<int, int> activationPixel);
    ...
    */
    ...
    void RegionGroup(std::vector<std::vector<std::pair<int, int>>> regionList, std::vector<std::vector<int>> &groupVec)
    {
        ...
        while (s.size() > 0) {
            ...
            s.erase(s.begin());
            if (s.size() == 0) {
                groupVec.push_back(std::vector<int> { m });
            } else {
    ...
    */
                            ...
                            std::vector<std::vector<std::pair<int, int>>> &regionList)
    {
        for (size_t i = 0; i < activationPixels.size(); i++) {
            ...
            if (merge == false) {
                regionList.push_back(std::vector<std::pair<int, int>> { activationPixels[i] });
            }
        }
    }
...
void DetectPostProcess(std::vector<std::shared_ptr<void>> outputDatas, uint32_t modelWidth, uint32_t modelHeight,
                       std::vector<std::vector<int>> &textBoxList)
{
    ...
    for (size_t i = 0; i < groupVec.size(); i++) {
        quadList.push_back(std::vector<int> { 0, 0, 0, 0, 0, 0, 0, 0 });
        scoreList.push_back(std::vector<float> { 0, 0, 0, 0 });
