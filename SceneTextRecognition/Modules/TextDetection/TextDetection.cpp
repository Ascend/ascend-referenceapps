...
APP_ERROR TextDetection::ParseConfig(ConfigParser &configParser)
{
...
APP_ERROR TextDetection::DeInit(void)
{
    ...
    if (modelProcess_ != nullptr) {
        ...
        if (ret != APP_ERR_OK) {
            LogFatal << "TextDetection[" << instanceId_ << "]: Failed to execute model DeInit, ret = " << ret << ".";
            return ret;
        }
    }
...
APP_ERROR TextDetection::Process(std::shared_ptr<void> inputData)
{
    ...
    std::vector<void *> inputBuffers { static_cast<uint8_t *>(sendData->resizedData.data.get()) };
