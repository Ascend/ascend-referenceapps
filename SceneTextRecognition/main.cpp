...
bool StopFlag::signalRecieved = false;
namespace {
...
int main(int argc, const char *argv[])
{
    ...
    ret = InitModuleManager(moduleManager, configPath);
    if (ret != APP_ERR_OK) {
        LogError << "Failed to init module manager, ret = " << ret << ".";
        exit(ret);
    }
    ...
    ret = moduleManager.RunPipeline();
    if (ret != APP_ERR_OK) {
        LogError << "Failed to run pipeline, ret = " << ret << ".";
        exit(ret);
    }
    ...
    ret = DeInitSystemManager(moduleManager);
    if (ret != APP_ERR_OK) {
        LogError << "Failed to deinit system manager, ret = " << ret << ".";
        exit(ret);
    }
