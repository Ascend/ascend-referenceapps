...
#include <ctime>
#include <exception>
#include <faiss/ascend/AscendIndexTSV2.h>
#include <functional>
...
namespace {
using idx_t = int64_t;
using FeatureAttrV2 = faiss::ascend::FeatureAttrV2;
using AttrFilterV2 = faiss::ascend::AttrFilterV2;
...
void FeatureAttrV2Generator(std::vector<FeatureAttrV2> &attrs)
{
    size_t n = attrs.size();
    for (size_t i = 0; i < n; ++i) {
...
TEST(TestAscendIndexTSV2, Init)
{
    uint32_t deviceId = 0;
    uint32_t dim = 512;
    uint32_t tokenNum = 2500;
    auto ts = GetMillisecs();
    faiss::ascend::AscendIndexTSV2 *tsIndex = new faiss::ascend::AscendIndexTSV2();
...
TEST(TestAscendIndexTSV2, add)
{
    ...
    std::vector<FeatureAttrV2>attrs(ntotal);
    FeatureAttrV2Generator(attrs);
    auto ts = GetMillisecs();
...
TEST(TestAscendIndexTSV2, GetFeatureByLabel)
{
    int dim = 512;
    int maxTokenId = 2500;
    int ntotal = 100000;
    ...
    std::vector<FeatureAttrV2> attrs(ntotal);
    FeatureAttrV2Generator(attrs);
    auto *index = new faiss::ascend::AscendIndexTSV2();
...
TEST(TestAscendIndexTSV2, DeleteFeatureByLabel)
{
    int dim = 512;
    int maxTokenId = 2500;
    int ntotal = 1000000;
...
TEST(TestAscendIndexTSV2, DeleteFeatureByToken)
{
    int dim = 512;
    int maxTokenId = 2500;
    int ntotal = 1000000;
...
TEST(TestAscendIndexTSV2, Acc)
{
    ...
    for (auto k :topks) {
        ...
        for (auto queryNum : queryNums) {
            ...
            std::vector<AttrFilterV2> queryFilters(queryNum, filter);
...
TEST(TestAscendIndexTSV2, SearchNoShareQPS)
{
    idx_t ntotal = 1000000;
    uint32_t addNum = 10;
    uint32_t deviceId = 0;
...
TEST(TestAscendIndexTSV2, SearchShareQPS)
{
    idx_t ntotal = 1000000;
    uint32_t addNum = 10;
    uint32_t deviceId = 0;
